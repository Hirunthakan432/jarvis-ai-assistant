...
def openai_loop(client, model, messages, registry, on_delta, cancel, on_event):
    ...
    for _ in range(MAX_ROUNDS):
        check_cancel(cancel)
        kwargs = dict(model=model, messages=transcript, tools=schemas)
        if on_delta:
...
def anthropic_loop(client, model, messages, registry, on_delta, cancel, on_event):
    ...
    for _ in range(MAX_ROUNDS):
        check_cancel(cancel)
        kwargs = dict(model=model, max_tokens=2048, system=messages[0]['content'], messages=transcript, tools=schemas)
        if on_delta:
